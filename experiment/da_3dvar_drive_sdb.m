...
function [analysis,xb_inc,xe_inc]=da_3dvar_drive_sdb(field,obs,prefix)
  ...
  for j=1:end_outer
    ...
    [increment, control ] = Control2State_sdb(control_y_all,Xp,prefix);
    ...
    for i=1:end_inner(j)
         ...
         gradient.y.static    = prefix.da.Bs * gradient.x.static;
         gradient.y.ensemble  = prefix.da.L  * ...
             reshape(gradient.x.ensemble,[prefix.model.main.resolution prefix.ensemblesize]);
          
         % Apply Scale-dependent Weighting 
         [ gradient.y ] = ApplySDB(gradient.y,prefix);
       
         fprintf(prefix.fid6a, '%10.6f',gradient.y.static );
         fprintf(prefix.fid6b, '%10.6f',gradient.y.ensemble );
        ...
        %control_f = gradient_y_all;
         ...
         [alpha,minCostFunc]= ...
           fminsearch(@CostFunc_sdb,0,options,control_y_all,control_d_y,Xp,prefix,innov,H); 
