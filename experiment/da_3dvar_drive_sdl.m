...
function [analysis,xb_inc,xe_inc]=da_3dvar_drive_sdl(field,obs,prefix)
  ...
  for j=1:end_outer
    ...
    [increment, control ] = Control2State_sdl(control_y_all,Xp,prefix);
    ...
    for i=1:end_inner(j)
         ...
         gradient.y.static.l 	= prefix.da.beta1_inv * prefix.da.Bs.l * gradient.x.static.l;
         gradient.y.static.s 	= prefix.da.beta1_inv * prefix.da.Bs.s * gradient.x.static.s;
         gradient.y.ensemble.l 	= prefix.da.beta2_inv * prefix.da.L.l  * ...
                  reshape(gradient.x.ensemble.l,[prefix.model.main.resolution prefix.ensemblesize]);
         gradient.y.ensemble.s 	= prefix.da.beta2_inv * prefix.da.L.s  * ...
                  reshape(gradient.x.ensemble.s,[prefix.model.main.resolution prefix.ensemblesize]);

         fprintf(prefix.fid6a, '%10.6f',gradient.y.static.l );
         fprintf(prefix.fid6a, '%10.6f',gradient.y.static.s );
         fprintf(prefix.fid6b, '%10.6f',gradient.y.ensemble.l );
         fprintf(prefix.fid6b, '%10.6f',gradient.y.ensemble.s );
        ...
        % control_f = gradient_y_all;
         ...
         [alpha,minCostFunc]= fminsearch(@CostFunc_sdl,0,options,control_y_all,control_d_y,Xp,prefix,innov,H); 
